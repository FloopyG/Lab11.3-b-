...
int main()
{
    ...
    do {
        ...
        case 4:
            ...
            break;
        case 5:
            cout << "Перевірка наявності студента за прізвищем, курсом, та оцінкою з фізики.";
...
int binSearch(const char* fName, const char* prizv, const int kurs, const int gradePhys) {
    ...
    do {
        m = (L + R) / 2;
        Student a = fRead(f, m);
        if (a.gradePhys == gradePhys && strcmp(a.prizv, prizv) == 0 && a.kurs == kurs) {
            return m + 1;
        }
        else if ((a.gradePhys < gradePhys) ||
            (a.gradePhys == gradePhys && strcmp(a.prizv, prizv) == -1) ||
            (a.gradePhys == gradePhys && strcmp(a.prizv, prizv) == 0 && a.kurs < kurs)) {
            R = m - 1;
        }
        else {
            L = m + 1;
